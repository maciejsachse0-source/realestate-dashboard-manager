 ...
 * Klient HTTP.
 *
 * Ciasteczko sesyjne jest HttpOnly, więc front go nie widzi ani nie ustawia.
 * Wystarczy `credentials: 'same-origin'` — przeglądarka dokłada je sama.
 */
...
export class BladApi extends Error {
  ...
  constructor(status: number, message: string, wersjaBiezaca: number | null = null) {
    super(message)
    this.name = 'BladApi'
    this.status = status
  ...
  /** Czy to znaczy „zaloguj się ponownie". */
  get wymagaLogowania(): boolean {
...
async function zadanie<T>(
  ...
  if (!odpowiedz.ok) {
    ...
    throw new BladApi(odpowiedz.status, komunikat, naglowek ? Number(naglowek) : null)
  }

  return (await odpowiedz.json()) as T
}

export function pobierz<T>(sciezka: string, params?: Record<string, unknown>): Promise<T> {
