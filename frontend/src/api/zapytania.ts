...
export const klucze = {
  ...
  stan: (lokalId: number, naDzien?: string) =>
    ["stan", lokalId, naDzien ?? "dzis"] as const,
  zdarzenia: (filtry: Record<string, unknown>) =>
    ["zdarzenia", filtry] as const,
  skan: ["skan"] as const,
};
...
export function useLokale(filtry: FiltryLokali) {
  return useQuery({
    ...
    queryFn: () =>
      pobierz<Strona<LokalNaLiscie>>(
        "/lokale",
        filtry as Record<string, unknown>,
      ),
...
export function useStanLokalu(lokalId: number, naDzien?: string) {
  return useQuery({
    ...
    queryFn: () =>
      pobierz<StanNaDzien>(`/lokale/${lokalId}/stan`, { na_dzien: naDzien }),
  });
}
...
export function useObsluzZdarzenie() {
  ...
  return useMutation({
    mutationFn: ({ id, notatka }: { id: number; notatka?: string }) =>
      wyslij<Zdarzenie>(`/zdarzenia/${id}/obsluzone`, {
...
export function useZabezpieczenia(okresId: number | null | undefined) {
  return useQuery({
    ...
    queryFn: () =>
      pobierz<Zabezpieczenie[]>(`/okresy-najmu/${okresId}/zabezpieczenia`),
    enabled: okresId != null,
  });
}
...
export function useDecyzjaOParametrze() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      wyslij<Parametr>(`/parametry/${id}/decyzja`, {
        status,
        uwagi: uwagi ?? null,
      }),
...
export function useDodajOkresNajmu() {
  ...
  return useMutation({
    mutationFn: (dane: Record<string, unknown>) =>
      wyslij<OkresNajmu>("/okresy-najmu", dane),
    onSuccess: () => {
...
export function useDodajSkladnik() {
  ...
  return useMutation({
    ...
    onSuccess: () =>
      void kolejka.invalidateQueries({ queryKey: ["skladniki"] }),
  });
}
...
export function useDodajPrzeglad() {
  ...
  return useMutation({
    mutationFn: (dane: Record<string, unknown>) =>
      wyslij<Przeglad>("/przeglady", dane),
    onSuccess: () =>
      void kolejka.invalidateQueries({ queryKey: ["przeglady"] }),
  });
}
...
export function useZaimportujZDysku() {
  ...
  return useMutation({
    mutationFn: (dane: Record<string, unknown>) =>
      wyslij<{ id: number }>("/skan/importuj", dane),
    onSuccess: () => {
...
export function useKatalogSkanu() {
  return useQuery({
    queryKey: ["katalog-skanu"],
    queryFn: () => pobierz<KatalogSkanu>("/skan/katalog"),
  });
}

export function useUstawKatalogSkanu() {
  const kolejka = useQueryClient();
  return useMutation({
    mutationFn: (sciezka: string) =>
      zapisz<KatalogSkanu>("/skan/katalog", { sciezka }),
    onSuccess: () => {
      void kolejka.invalidateQueries({ queryKey: ["katalog-skanu"] });
      void kolejka.invalidateQueries({ queryKey: klucze.skan });
    },
  });
}
