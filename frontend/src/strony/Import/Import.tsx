...
export default function Import() {
   ...
   */
  async function wyslij(sciezka: string, mapa: Record<string, string>): Promise<Response> {
  ...
  async function wczytajPodglad(mapa: Record<string, string> = mapowanie) {
    ...
    try {
      ...
      if (!odpowiedz.ok) {
        setBlad(typeof tresc.detail === 'string' ? tresc.detail : 'Nie udało się odczytać pliku.')
        setPodglad(null)
        return
      }
      ...
      if (Object.keys(mapa).length === 0) {
        const zgadniete = Object.fromEntries(dane.naglowki.map((n) => [n, zgadnijPole(n)]))
    ...
    try {
      ...
      if (!odpowiedz.ok) {
        if (tresc.detail?.bledy) {
          ...
          setBlad(
            typeof tresc.detail === 'string' ? tresc.detail : 'Import się nie powiódł.',
          )
        }
        return
      }
  ...
  if (wynik) {
    return <Zakonczenie wynik={wynik} onOdNowa={() => { setWynik(null); setPlik(null); setPodglad(null); setMapowanie({}) }} />
  }

  const gotowyDoImportu = podglad !== null && podglad.bledy.length === 0 && podglad.wierszy_poprawnych > 0
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Wgraj arkusz, sprawdź, jak system go zrozumiał, a potem zapisz.
          Import zapisuje albo cały plik, albo nic.
        </p>
      </div>
      ...
      {podglad && (
        <>
          <section className="space-y-3 rounded-lg border bg-background p-4">
            <h2 className="text-sm font-medium">Krok 2: co jest w której kolumnie</h2>
          ...
          <section className="space-y-3">
            <h2 className="text-sm font-medium">
              Krok 3: podgląd ({podglad.wierszy_poprawnych} z {podglad.wierszy_w_pliku} wierszy
              gotowych)
            </h2>
            ...
            {podglad.bledy.length > 0 && (
              <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-3">
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  Popraw arkusz i wgraj go ponownie. Dopóki jest choć jeden problem,
            ...
            <div className="flex items-center gap-3">
              <Button onClick={() => void zaimportuj()} disabled={!gotowyDoImportu || pracuje}>
                {pracuje ? 'Importuję…' : `Zapisz ${podglad.wierszy_poprawnych} wierszy`}
              </Button>
...
function Zakonczenie({ wynik, onOdNowa }: { wynik: WynikImportu; onOdNowa: () => void }) {
          ...
          <tbody>
            {pozycje.map(([nazwa, ile]) => (
              <tr key={nazwa} className="border-b last:border-b-0">
                ...
                <td className="px-3 py-2 text-right tabular-nums font-medium">{ile}</td>
              </tr>
            ))}
          </tbody>
        </table>
