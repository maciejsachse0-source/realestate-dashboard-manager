...
export default function Skan() {
  ...
  if (skan.isError) {
    return (
      <Blad
        komunikat={skan.error instanceof Error ? skan.error.message : 'Nieznany błąd.'}
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            {dane.katalog ?? 'Katalog nie jest ustawiony'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button onClick={() => void skan.refetch()} disabled={skan.isFetching}>
      ...
      {!dane.dostepny && (
        <Pusto
          ...
          opis={dane.komunikat ?? 'Katalog z dokumentami jest niedostępny.'}
        />
      )}

      {sprawdzam && przeglad.data && <PrzegladOdnosnikow dane={przeglad.data} />}
      ...
      {dane.budynki.map((budynek) => (
        <section key={budynek.nazwa_folderu} className="space-y-3">
          ...
          {budynek.foldery.length === 0 ? (
            <p className="text-sm text-muted-foreground">Brak folderów lokali w tym budynku.</p>
          ) : (
            budynek.foldery.map((folder) => <Folder key={folder.sciezka_wzgledna} folder={folder} />)
          )}
        </section>
      ))}
    </div>
...
function PrzegladOdnosnikow({
  ...
  dane: { sprawdzonych: number; zerwane: { dokument_id: number; nazwa: string | null; sciezka_wzgledna: string; powod: string }[] }
}) {
  ...
  return (
    ...
    >
      ...
      <p className="text-muted-foreground">
        Dokumenty nie są kopiowane, więc przeniesienie albo przemianowanie pliku w Eksploratorze
        zrywa odnośnik. Przywróć plik pod starą ścieżkę albo dodaj go jeszcze raz.
      </p>
      <ul className="space-y-1">
        {dane.zerwane.map((z) => (
          <li key={z.dokument_id}>
            <span className="font-medium">{z.nazwa ?? `dokument #${z.dokument_id}`}</span>
            <span className="text-muted-foreground"> — {z.powod}</span>
            <div className="text-xs text-muted-foreground">{z.sciezka_wzgledna}</div>
          </li>
        ))}
      </ul>
    </div>
...
function Folder({ folder }: { folder: FolderZeSkanu }) {
  ...
  const doWyboru = (lokale.data?.pozycje ?? []).filter((l) => l.okres_najmu_id !== null)
    ...
    <div className="rounded-lg border">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b bg-muted/40 px-4 py-2.5">
        <div>
          ...
          {folder.opis_umowy ? (
            <p className="text-sm text-muted-foreground">Umowa: {folder.opis_umowy}</p>
          ) : (
            <p className="text-sm text-muted-foreground">
            ...
            >
              ...
              {doWyboru.map((l) => (
                <option key={l.okres_najmu_id} value={String(l.okres_najmu_id)}>
                  {l.budynek_nazwa} / {l.oznaczenie} — {l.najemca_nazwa ?? 'bez najemcy'}
                </option>
              ))}
            </select>
            <Button
      ...
      {folder.pliki.length === 0 ? (
        ...
        <ul className="divide-y">
          {folder.pliki.map((plik) => (
            <Plik key={plik.sciezka_wzgledna} plik={plik} okresNajmuId={folder.okres_najmu_id} />
          ))}
        </ul>
      )}
    </div>
...
function Plik({ plik, okresNajmuId }: { plik: PlikZeSkanu; okresNajmuId: number | null }) {
  ...
  return (
    <li className="space-y-2 px-4 py-3">
      <div className="flex flex-wrap items-center gap-3">
        <span className="min-w-64 flex-1 text-sm font-medium">{plik.nazwa}</span>
        ...
        <Button
          ...
          onClick={() => {
            ...
            importuj.mutate(
              ...
              {
                onError: (e) => setBlad(e instanceof Error ? e.message : 'Nie udało się dodać.'),
              },
            )
          }}
        >
        ...
        <Button
          variant="ghost"
          size="sm"
          onClick={() => pomin.mutate({ sciezka_wzgledna: plik.sciezka_wzgledna })}
        >
          Pomiń
        </Button>
      </div>
      ...
      {okresNajmuId === null && (
        <p className="text-xs text-muted-foreground">
          Najpierw powiąż ten folder z umową — dokument musi wiedzieć, do czego należy.
        </p>
      )}
