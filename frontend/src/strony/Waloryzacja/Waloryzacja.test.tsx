...
beforeEach(() => {
  ...
  vi.stubGlobal("fetch", (adres: string, opcje?: RequestInit) => {
    const tresc = opcje?.body
      ? (JSON.parse(String(opcje.body)) as Record<string, never>)
      : {};
    ...
    if (adres.includes("/waloryzacja/zatwierdz")) {
      if (konflikt) {
        return Promise.resolve(
          new Response(
            JSON.stringify({ detail: "Odśwież listę propozycji." }),
            {
              status: 409,
              headers: { "Content-Type": "application/json" },
            },
      ...
      return odpowiedz({
        rok: ROK,
        umow_zwaloryzowanych: zatwierdzone.length,
        zdarzen_o_wekslach: 1,
      });
    }
...
describe("Ekran waloryzacji", () => {
  ...
  it("pokazuje wyłączone umowy razem z powodem", async () => {
    ...
    expect(
      screen.getByText("Umowa nie podlega waloryzacji."),
    ).toBeInTheDocument();
  });
  ...
  it("domyślnie zaznacza wszystko", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /Zatwierdź 2 umowy/ }),
    ).toBeEnabled();
  });
  ...
  it("odznaczenie umowy zmienia sumę, bo pyta o nią serwer", async () => {
    ...
    await screen.findByText("A/01");
    await waitFor(() =>
      expect(screen.getByText(/Razem PLN \(2 umowy\)/)).toBeInTheDocument(),
    );

    await uzytkownik.click(
      screen.getByRole("checkbox", { name: "Zatwierdź A/02" }),
    );

    await waitFor(() =>
      expect(screen.getByText(/Razem PLN \(1 umowę\)/)).toBeInTheDocument(),
    );
    ...
    expect(
      screen.getByRole("button", { name: /Zatwierdź 1 umowę/ }),
    ).toBeInTheDocument();
  });
  ...
  it("zatwierdza tylko zaznaczone umowy", async () => {
    ...
    await screen.findByText("A/01");
    await uzytkownik.click(
      screen.getByRole("checkbox", { name: "Zatwierdź A/01" }),
    );
    await uzytkownik.click(
      screen.getByRole("button", { name: /Zatwierdź 1 umowę/ }),
    );
  ...
  it("po zatwierdzeniu mówi o zdarzeniach o zabezpieczeniach", async () => {
    ...
    expect(
      await screen.findByText(/Zwaloryzowano 2 umowy/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/zabezpieczeniach do przeliczenia/),
    ).toBeInTheDocument();
  });
  ...
  it("po konflikcie 409 sam pobiera listę na nowo", async () => {
    ...
    expect(
      await screen.findByText("Odśwież listę propozycji."),
    ).toBeInTheDocument();
  ...
  it("odznaczenie wszystkiego blokuje zatwierdzenie", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Nic nie zaznaczono — nie ma czego zatwierdzić."),
      ).toBeInTheDocument(),
    );
    expect(
      screen.getByRole("button", { name: /Zatwierdź 0 umów/ }),
    ).toBeDisabled();
  });
});
...
describe("Ekran waloryzacji: pole roku", () => {
  it("skasowanie roku nie wysyła zapytania z rok=0", async () => {
    ...
    vi.stubGlobal("fetch", (adres: string) => {
      ...
      if (adres.includes("/waloryzacja/podsumowanie"))
        return odpowiedz(PRZEBIEG.sumy);
      throw new Error(`Nieoczekiwane żądanie: ${adres}`);
    });
    ...
    expect(
      await screen.findByText("Podaj rok waloryzacji"),
    ).toBeInTheDocument();
