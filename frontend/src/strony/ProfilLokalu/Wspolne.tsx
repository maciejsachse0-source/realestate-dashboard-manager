...
export function Pole({ etykieta, children }: { etykieta: string; children: ReactNode }) {
  ...
  return (
    <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 border-b px-3 py-2.5 last:border-b-0">
      <span className="w-56 shrink-0 text-sm text-muted-foreground">{etykieta}</span>
      <span className="text-sm">
        {puste ? <span className="text-muted-foreground">{BRAK_DANYCH}</span> : children}
      </span>
    </div>
  )
}

export function Karta({ tytul, children }: { tytul?: string; children: ReactNode }) {
  return (
    <section className="rounded-lg border bg-background">
      {tytul && <h2 className="border-b px-3 py-2 text-sm font-medium">{tytul}</h2>}
      {children}
    </section>
  )
}
...
export function OdznakaWeryfikacji({ status }: { status: string }) {
  ...
  return (
    <span className={`rounded border px-1.5 py-0.5 text-xs ${kolor}`} title="Nie wchodzi do stanu efektywnego">
      {status}
    </span>
  )
}
