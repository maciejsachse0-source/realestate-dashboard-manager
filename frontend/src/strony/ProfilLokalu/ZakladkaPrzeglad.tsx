    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Karta tytul="Lokal">
        ...
        <Pole etykieta="Status">
          {ETYKIETY_STATUSU[lokal.status] ?? lokal.status}
        </Pole>
        ...
        {okres === null ? (
          <div className="px-3 py-8 text-center">
            <p className="text-sm text-muted-foreground">
              Lokal nie ma bieżącej umowy najmu.
            </p>
          ...
          <>
            <Pole etykieta="Status">
              {ETYKIETY_STATUSU[okres.status] ?? okres.status}
            </Pole>
            <Pole etykieta="Data zawarcia">
              {formatujDate(okres.data_zawarcia)}
            </Pole>
            <Pole etykieta="Data przekazania">
              {formatujDate(okres.data_przekazania)}
            </Pole>
            <Pole etykieta="Okres">
              {okres.okres_zawarcia_miesiace
                ? `${okres.okres_zawarcia_miesiace} mies.`
                : null}
            </Pole>
      ...
      <Karta tytul="Warunki obowiązujące">
        {Object.keys(stan.parametry).length === 0 ? (
          <p className="px-3 py-6 text-center text-sm text-muted-foreground">
            Brak zatwierdzonych warunków. Wartości czekające na decyzję są w
            zakładce Historia.
          </p>
        ) : (
          Object.entries(stan.parametry).map(([klucz, wartosc]) => (
            <Pole
              key={klucz}
              etykieta={NAZWY_PARAMETROW[klucz] ?? czytelnaNazwaPola(klucz)}
            >
