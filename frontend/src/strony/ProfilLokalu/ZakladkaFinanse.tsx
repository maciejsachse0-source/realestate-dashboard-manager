...
export function ZakladkaFinanse({
  ...
  if (okresId === null) {
    return (
      <Pusto tytul="Brak umowy" opis="Bez umowy nie ma składników opłat." />
    );
  }
  ...
  if (skladniki.isError) {
    return (
      <Blad
        komunikat={
          skladniki.error instanceof Error
            ? skladniki.error.message
            : "Nieznany błąd."
        }
            ...
            {skladniki.data.map((s) => {
              ...
              return (
                <tr key={s.id} className="border-b last:border-b-0">
                  ...
                  <td className="px-3 py-2.5 text-right tabular-nums">
                    {wartosc?.typ === "kwota" ? (
                      ...
                      <span
                        ...
                        title="Kwota nie jest zatwierdzona"
                      >
                        nieustalona
                      </span>
                    )}
                  ...
                  <td className="px-3 py-2.5">
                    {s.dzien_platnosci_miesiaca
                      ? `do ${s.dzien_platnosci_miesiaca}-go`
                      : "—"}
                  </td>
                  <td className="px-3 py-2.5">
                    {s.dzien_platnosci_roboczy
                      ? formatujDate(s.dzien_platnosci_roboczy)
                      : "—"}
                  </td>
                  <td className="px-3 py-2.5">
                    {s.czy_waloryzowany ? "tak" : "nie"}
                  </td>
                </tr>
              );
            })}
