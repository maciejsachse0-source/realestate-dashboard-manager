...
export function ZakladkaZabezpieczenia({ okresId }: { okresId: number | null }) {
  ...
  if (zabezpieczenia.isError) {
    return (
      <Blad
        komunikat={
          zabezpieczenia.error instanceof Error ? zabezpieczenia.error.message : 'Nieznany błąd.'
        }
    ...
    return (
      <>
        <Pusto
          ...
          akcja={<Button onClick={() => setOtwarty(true)}>Dodaj zabezpieczenie</Button>}
        />
          ...
          <div className="flex flex-wrap items-center gap-x-8 gap-y-3 px-3 py-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm tabular-nums">
                {z.wymagana_wartosc
                  ? formatujKwote(z.wymagana_wartosc, z.wymagana_waluta ?? 'PLN')
                  : z.sposob_wyliczenia || '—'}
              </p>
            </div>

            <div>
              <p className="text-xs text-muted-foreground">Termin dostarczenia</p>
              <p className="mt-1 text-sm">{formatujDate(z.data_wymagalnosci)}</p>
            </div>

            <div>
              <p className="text-xs text-muted-foreground">Dostarczono</p>
              <p className="mt-1 text-sm">{formatujDate(z.data_dostarczenia)}</p>
            </div>
