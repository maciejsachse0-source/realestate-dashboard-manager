...
describe("formatujKwote", () => {
  it("uzywa przecinka dziesietnego i spacji jako separatora tysiecy", () => {
    ...
    expect(formatujKwote("1234.56").replace(/\u00a0|\u202f/g, " ")).toBe(
      "1 234,56 zł",
    );
  });

  it("zawsze pokazuje dwa miejsca po przecinku", () => {
    expect(formatujKwote(1000).replace(/\u00a0|\u202f/g, " ")).toBe(
      "1 000,00 zł",
    );
  });

  it("grupuje takze setki tysiecy", () => {
    expect(formatujKwote("987654.32").replace(/ | /g, " ")).toBe(
      "987 654,32 zł",
    );
  });
...
describe("formatujPowierzchnie", () => {
  it("formatuje z jednostka", () => {
    expect(formatujPowierzchnie("128.5").replace(/\u00a0|\u202f/g, " ")).toBe(
      "128,50 m²",
    );
  });
});
