...
class TestListaLokali:
    def test_pusty_wynik_ma_poprawny_ksztalt(self, klient_podglad: TestClient) -> None:
        dane = klient_podglad.get("/api/v1/lokale").json()
        assert dane["pozycje"] == []
        assert dane["wszystkich"] == 0

    def test_lokal_bez_umowy(self, klient_podglad: TestClient, lokal_api: Lokal) -> None:
        wiersz = klient_podglad.get("/api/v1/lokale").json()["pozycje"][0]
    ...
    def test_lokal_z_umowa_i_czynszem(
        self, klient_podglad: TestClient, baza: Session, umowa: OkresNajmu
    ) -> None:
        dodaj_czynsz(baza, umowa, "12500.00", date(2026, 2, 1))
        wiersz = klient_podglad.get(f"/api/v1/lokale?na_dzien={DZIS}").json()["pozycje"][0]
    ...
    def test_brak_protokolu_daje_powod_zamiast_daty(
        self, klient_podglad: TestClient, baza: Session, lokal_api: Lokal, najemca_api: Najemca
    ) -> None:
...
class TestFiltryISortowanie:
    def test_filtr_po_budynku(
        self, klient_podglad: TestClient, baza: Session, lokal_api: Lokal, budynek_api: Budynek
    ) -> None:
        ...
        assert (
            klient_podglad.get(f"/api/v1/lokale?budynek_id={budynek_api.id}").json()["wszystkich"]
            == 1
        )
        assert klient_podglad.get(f"/api/v1/lokale?budynek_id={inny.id}").json()["wszystkich"] == 0

    def test_filtr_po_zakresie_konca_umowy(
        self, klient_podglad: TestClient, umowa: OkresNajmu
    ) -> None:
        """Pytanie z sekcji 1.1 koncepcji: 'umowy kończące się w Q2 2027'."""
        w_zakresie = klient_podglad.get(
            "/api/v1/lokale?koniec_od=2028-01-01&koniec_do=2028-12-31"
        ).json()
        poza = klient_podglad.get("/api/v1/lokale?koniec_od=2027-04-01&koniec_do=2027-06-30").json()
        assert w_zakresie["wszystkich"] == 1
        assert poza["wszystkich"] == 0

    def test_wyszukiwanie_po_najemcy(self, klient_podglad: TestClient, umowa: OkresNajmu) -> None:
        assert klient_podglad.get("/api/v1/lokale?szukaj=Przykładowa").json()["wszystkich"] == 1
        assert klient_podglad.get("/api/v1/lokale?szukaj=NieMaTakiego").json()["wszystkich"] == 0

    def test_wyszukiwanie_po_oznaczeniu(self, klient_podglad: TestClient, lokal_api: Lokal) -> None:
        assert klient_podglad.get("/api/v1/lokale?szukaj=18A/12").json()["wszystkich"] == 1

    def test_nieznana_kolumna_sortowania_jest_odrzucana(self, klient_podglad: TestClient) -> None:
        """Biała lista kolumn. Do ORDER BY nie trafia dowolny tekst."""
        odpowiedz = klient_podglad.get("/api/v1/lokale?sortuj=DROP+TABLE")
        assert odpowiedz.status_code == 422
        assert "Dostępne" in odpowiedz.json()["detail"]

    def test_paginacja(self, klient_podglad: TestClient, lokal_api: Lokal) -> None:
        dane = klient_podglad.get("/api/v1/lokale?limit=1&offset=0").json()
        assert dane["limit"] == 1
        assert len(dane["pozycje"]) <= 1

    def test_limit_ponad_maksimum_jest_odrzucany(self, klient_podglad: TestClient) -> None:
        assert klient_podglad.get("/api/v1/lokale?limit=5000").status_code == 422


class TestStanNaDzien:
    def test_stan_pokazuje_wartosc_z_danego_dnia(
        self, klient_podglad: TestClient, baza: Session, umowa: OkresNajmu, lokal_api: Lokal
    ) -> None:
        """Decyzja D2 przez cały stos: 'jaka była stawka w maju 2026'."""
        dodaj_czynsz(baza, umowa, "12500.00", date(2026, 2, 1))
        dodaj_czynsz(baza, umowa, "13000.00", date(2027, 3, 1))

        maj = klient_podglad.get(f"/api/v1/lokale/{lokal_api.id}/stan?na_dzien=2026-05-15").json()
        pozniej = klient_podglad.get(
            f"/api/v1/lokale/{lokal_api.id}/stan?na_dzien=2027-06-01"
        ).json()
    ...
    def test_stan_niesie_slad_do_dokumentu(
        ...
        pozycja = klient_podglad.get(f"/api/v1/lokale/{lokal_api.id}/stan?na_dzien={DZIS}").json()[
            "parametry"
        ]["czynsz_podstawowy"]
    ...
    def test_lokal_bez_umowy_ma_zerowa_kompletnosc(
        self, klient_podglad: TestClient, lokal_api: Lokal
    ) -> None:
        dane = klient_podglad.get(f"/api/v1/lokale/{lokal_api.id}/stan").json()
    ...
    def test_nieistniejacy_lokal_daje_404(self, klient_podglad: TestClient) -> None:
        assert klient_podglad.get("/api/v1/lokale/999999/stan").status_code == 404


class TestKokpitTerminow:
    def test_generator_i_lista_zdarzen(
        self, klient_zarzadca: TestClient, baza: Session, lokal_api: Lokal, najemca_api: Najemca
    ) -> None:
        baza.add(
            OkresNajmu(
                lokal_id=lokal_api.id,
        ...
        przebieg = klient_zarzadca.post(f"/api/v1/zdarzenia/generuj?na_dzien={DZIS}")
        assert przebieg.status_code == 200
        assert przebieg.json()["zdarzen_dodanych"] > 0

        lista = klient_zarzadca.get("/api/v1/zdarzenia").json()
    ...
    def test_krytyczne_sa_na_gorze_listy(
        ...
        baza.flush()
        klient_zarzadca.post(f"/api/v1/zdarzenia/generuj?na_dzien={DZIS}")

        wagi = [z["waga"] for z in klient_zarzadca.get("/api/v1/zdarzenia").json()["pozycje"]]
    ...
    def test_obsluzenie_zdarzenia(
        ...
        pierwsze = klient_zarzadca.get("/api/v1/zdarzenia").json()["pozycje"][0]
    ...
    def test_licznik_zdarzen_na_liscie_lokali(
        ...
        wiersz = klient_zarzadca.get("/api/v1/lokale").json()["pozycje"][0]
